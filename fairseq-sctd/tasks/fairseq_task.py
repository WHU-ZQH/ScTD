...
class FairseqTask(object):
    ...
    def train_step(
        self, sample, model, criterion, optimizer, update_num, ignore_grad=False, rank=None
    ):
        """
        ...
        with torch.autograd.profiler.record_function("forward"):
            with torch.cuda.amp.autocast(enabled=(isinstance(optimizer, AMPOptimizer))):
                if update_num%10==0:
                    loss, sample_size, logging_output, mlm_losses, mlm_ids = criterion(model, sample, record_mlm_loss=True, rank=rank, align=True)
                else:
                    loss, sample_size, logging_output, mlm_losses, mlm_ids = criterion(model, sample, record_mlm_loss=True, rank=rank, align=False)
                model.encoder.sentence_encoder.record_mlm_loss(mlm_ids, mlm_losses)
