...
class RobertaModel(FairseqEncoderModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument(
            "--token-drop", type=float, metavar="D", default=0.0, help="token dropout probability"
        )
        parser.add_argument(
            "--init-token-importance", type=str, metavar="D", default=None, help="token dropout probability"
        )
        parser.add_argument(
            "--start-layer", type=int, metavar="D", default=11, help="token dropout probability"
        )
        parser.add_argument(
            "--attention-dropout",
            type=float,
    ...
    def forward(
        ...
        **kwargs,
        ...
        x, extra = self.encoder(src_tokens, features_only, return_all_hiddens,rank=rank, **kwargs)
    ...
    def forward(
        ...
        **unused,
        ...
        x, extra = self.extract_features(
            src_tokens, return_all_hiddens=return_all_hiddens, rank=rank
        )
        if not features_only:
            x = self.output_layer(x, masked_tokens=masked_tokens)
        return x, extra

    def extract_features(self, src_tokens, return_all_hiddens=False,rank=None, **kwargs):
